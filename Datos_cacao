...
library(tidyverse)   
library(janitor)
setwd("D:/UBA/Econometria II/TP")
getwd()
rm(list = ls())

precio_productor <- read.csv("precio_productor_ghana_cdi_1991_2024.csv") %>% clean_names() 
datos_exportacion <- read.csv("Cocoa exports Gh CD.csv") %>% clean_names()
datos_produccion <- read.csv("Cocoa beans production Gh y CdI.csv") %>% clean_names()
precio_anual_nominal <- read_excel("precio_anual_por_kg.xlsx", 
                    sheet = "Annual Prices (Nominal)",
                    skip = 8,
                    col_names = c("year", "precio_kg_nominal")) %>% clean_names()
precio_anual_real <- read_excel("precio_anual_por_kg.xlsx", sheet = "Annual Prices (Real)",
                                skip = 8,
                                col_names = c("year", "precio_kg_real")) %>% clean_names()
tipo_cambio <- read_excel("Tipo de cambio oficial y real G y CdV 1991-2025.xlsx") %>% clean_names()
derivados <- read.csv("derivados_cocoa.csv") %>% clean_names()
